...
<svelte:head>
	<title>Workspace User: {workspaceUser.user.name} - {workspace.name} - {APP_NAME}</title>
</svelte:head>
<RouteBreadcrumbs
	crumbs={[
...
/>
<main id="main">
	<RouteHeader heading="Workspace User" subheading={workspaceUser.user.name} />
	<RouteSection id="workspace-user">
		<TwoColumns>
			...
			{#snippet right()}
				<Button href="/workspaces/{workspace.id}/users/{workspaceUser.user.id}/remove"
					>Remove User</Button
				>
			{/snippet}
		</TwoColumns>
